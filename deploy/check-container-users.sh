...
# Script to check users in container database
# Run this on the remote server

CONTAINER_NAME="barcode-generator-api"
...
if ! docker ps | grep -q "$CONTAINER_NAME"; then
    ...
    docker ps --format "{{.Names}}" | grep -E "(barcode|api)" || echo "  (none found)"
    exit 1
fi
...
if [ -z "$DATABASE_PATH" ]; then
    echo "   ⚠️  DATABASE_PATH not set (using default)"
